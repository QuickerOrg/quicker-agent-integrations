...
if ($Check) {
    ...
    try {
        $pending = $tcpClient.BeginConnect('127.0.0.1', $settings.Port, $null, $null)
...
while ($null -ne ($line = [Console]::In.ReadLine())) {
    ...
    try {
        ...
        $request.Headers['Mcp-Client-Info'] = ($Client + '-quicker-plugin')
        ...
        if ($status -lt 200 -or $status -ge 300) {
            ...
            elseif ($status -eq 403) { $failureCode = 'client_not_approved'; $message = 'Approve the connecting MCP client in Quicker, then reconnect.' }
