...
让 Codex 通过本机 Quicker 编写、保存并预览自动化动作。版本 0.1.0，MIT 许可证。
...
运行 Quicker Debug，在设置中启用 MCP 和「允许 MCP 写入」，然后新建 Codex 任务。首次连接在 Quicker 的客户端同意窗口确认所显示的客户端。当前 Quicker Release 尚不提供 MCP。
